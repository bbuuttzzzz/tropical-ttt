...
local function DrawMapIcon(parent, mapname)
  ...
  if file.Exists("tropical/mapicons/" .. mapname .. ".jpg","DATA") then
    ...
    local githuburl = "https://orecros.github.io/tropical-map-icons/" .. mapname .. ".jpg"
...
function GM:OpenMapVote()
  ...
  self.MapVotePanel = self:MakeMapViewer("Vote for a Map", function(entryTab)
    RunConsoleCommand("ter_votemap",entryTab.MapName)
  end)
...
function GM:UpdateMapVoteCounts()
  ...
  for map, tab in pairs(self.MapVotePanel.MapEntries) do
    tab.VoteFrac = (self.MapVotes[map] or 0) / numVotes
  end
end
...
function GM:MakeMapViewer(topText, ClickCallBack)
  ...
  local textLabel = EasyLabel(topSpace,topText,"ZSHUDFontSmall",COLOR_WHITE)
  textLabel:CenterHorizontal()
  textLabel:CenterVertical()

  --create the close button
  local button = EasyButton(topSpace, "Close", 8, 4)
  ...
  for mapname, mapTab in pairs(self.MapTable) do
    ...
    --draw the name below
    local nameTag = EasyLabel(base,mapname,"ZSHUDFontSmallest", COLOR_WHITE)
    ...
    for i, tag in ipairs(mapTab.tags) do
      ...
      local tagText = EasyLabel(tagFrame,MAPTAG_NAMES[tag] or "!badtag!","ZSHUDFontSmallest",COLOR_WHITE)
