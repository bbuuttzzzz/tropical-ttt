local MAP_TABLE_FILENAME = "tropicalmaptable.txt"
GM.MapNominations = {}
util.AddNetworkString("tter_votemap_start")
util.AddNetworkString("tter_votemap_update")
util.AddNetworkString("tter_forcemap_open")
...
hook.Add("LoadNextMap", "TropicalLoadNextMap", loadTropicalMap)
function GM:TropicalLoadNextMap()
  self.MapVoteInProgress = true
  self.MapVotes = {}

  timer.Simple(self.MapVoteTime, function()
    self:PostLoadNextMap()
  end)
  timer.Simple(self.MapVoteTime + 1, function()
    RunConsoleCommand("changelevel", self.WinningMap or game.GetMap())
  end)
  timer.Simple(self.MapVoteTime + 2, function()
    RunConsoleCommand("changelevel", game.GetMap())
  end)

  --tell everyone it's time to votemap
  net.Start("tter_votemap_start")
  net.Broadcast()
end
...
function GM:LoadMapList()
  if not file.Exists(MAP_TABLE_FILENAME, "DATA") then
    print("error: map table file not found")
    return {}
  end

  local contents = file.Read(MAP_TABLE_FILENAME, "DATA")
...
function GM:SaveMapList(maplist)
  file.CreateDir(string.GetPathFromFilename(MAP_TABLE_FILENAME))
  file.Write(MAP_TABLE_FILENAME, Serialize(maplist))
end
...
concommand.Add("zs_votemap", function(sender, command, arguments)
